...
public final class RoleCardClient {
    ...
    @SubscribeEvent public static void registerKey(RegisterKeyMappingsEvent event) { event.register(OPEN_KEY); ClientHooks.install(ClientCardCache::update); ClientHooks.installPublicName(ClientDisplayNames::update); ClientHooks.installAdmin((id, name, card) -> Minecraft.getInstance().setScreen(new AdminRoleCardScreen(id, name, card))); }
