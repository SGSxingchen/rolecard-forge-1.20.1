...
public record AdminSaveCardPacket(UUID target, long revision, String name, int age, String gender, String biography, int points, int[] stats) {
    ...
    static void handle(AdminSaveCardPacket p,Supplier<NetworkEvent.Context>s){NetworkEvent.Context c=s.get();ServerPlayer admin=c.getSender();if(admin!=null)c.enqueueWork(()->{if(!admin.hasPermissions(2)){admin.sendSystemMessage(Component.literal("没有管理权限。"));RoleCardNetwork.feedback(admin, "权限不足：没有管理权限。", true);return;}ServerPlayer target=admin.server.getPlayerList().getPlayer(p.target);if(target==null){admin.sendSystemMessage(Component.literal("目标玩家当前不在线。"));RoleCardNetwork.feedback(admin, "目标玩家当前不在线。", true);return;}target.getCapability(RoleCardCapability.CAPABILITY).ifPresent(card->{if(!RoleCardService.adminSave(target,card,p.revision,p.name,p.age,p.gender,p.biography,p.points,p.stats)){admin.sendSystemMessage(Component.literal("保存失败：数据非法或版本已变化。"));RoleCardNetwork.feedback(admin, "保存失败：数据非法或修订号已变化。", true);}else RoleCardNetwork.openAdmin(admin,target,card);});});c.setPacketHandled(true);}
}
